...
<field-group>
<field name="formConfigs!" type="FormController.Configs<TOutput>">

The configuration object passed to internal `FormController` constructor.
It defines the initial data, validation schema, and equality comparators for the form.

</field>

<field name="hookConfigs.validateMode?:" type="'onChange' | 'onBlur' | 'onSubmit'">
...
<field name="hookConfigs.revalidateMode?:" type="'onChange' | 'onBlur' | 'onSubmit'">
...
- 📦 [`<FieldRenderer />`](/react/fieldrenderer): A **declarative reactive boundary** that *automatically subscribes* to the **field it renders**. This is the **preferred way** to build your forms.
- ⚓ [`useFormField()`](/react/useformfield): Establishes a **scoped connection** to a **specific field**, giving you the `field` API **without** subscribing to *unrelated changes*.
- ⚓ [`useFieldValue()`](/react/usefieldvalue): Reactively tracks only that **field's value** — *nothing else in the form can trigger a re-render*.
- ⚓ [`useFieldIssues()`](/react/usefieldissues): Reactively tracks **validation state** for a **single field**.
- 👁‍🗨 [`form.watchValues()`](#watchvalues): *Re-renders* the calling component when **any form value** changes. Useful for **previews**, **summaries**, or **debugging views**.
- 👁‍🗨 [`form.watchIssues()`](#watchissues): *Re-renders* when **validation issues change**. Ideal for **global error banners** or **submit-state UI**.
- 👁‍🗨 [`form.watchEvent()`](#watchevent): Lets you *subscribe* to any **internal controller event** with **full control**.
