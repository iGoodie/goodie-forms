...
- :icon{name="material-symbols:readiness-score-outline" class="size-5 text-secondary"} A **framework-agnostic core** handles **state**, **validation**, **paths**, and **events**.
- :icon{name="tabler:layout-board-split" class="size-5 text-secondary"} **UI bindings** (_React_, _Vue_, or _any future adapter_) act purely as **subscription layers**.
