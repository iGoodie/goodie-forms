...
seo:
  title: FieldPath API Documentation — Type-Safe Deep Object Path Utilities for TypeScript
  description: Explore FieldPath, a powerful type-safe deep object path system for TypeScript. Learn how to read, write, modify, delete, and validate deeply nested values with full compile-time inference, string path support, and intelligent array handling.
---
...
— enabling full _compile-time validation_ and _deep type inference_ when working with **string-based field paths**.
...
::doc-ref-section
```ts
export function toStringPath(path: FieldPath.Segments);
```

Converts a `Segments` path into its **canonical string representation**.

It **serializes** object keys using **dot notation** and array indices using **bracket notation** — _producing a normalized `StringPath` suitable for display, logging, storage, or public APIs._
...
::doc-ref-section
```ts
export function fromStringPath<TStrPath extends string>(stringPath: TStrPath);
```
:::doc-tree
...
::doc-ref-section
```ts
export function equals(path1?: FieldPath.Segments, path2?: FieldPath.Segments);
```
...
No **normalization** or **coercion** is performed — _comparison is structural and exact._
...
This is a **prefix check** — _not a deep comparison._
