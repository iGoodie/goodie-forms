...
seo:
  title: FieldRenderer – Typed React Field Binding | Goodie Forms
  description: Detailed reference for the FieldRenderer component in Goodie Forms. Learn how to bind strongly-typed fields to React with full control, minimal abstraction, and explicit validation behavior.
---
...
`FieldRenderer` is the _React_ **field binding layer** of _Goodie Forms_.

It connects a **single field** (_resolved via a strongly-typed path_) to **your UI** — _without introducing implicit magic, hidden state, or uncontrolled behavior._
`FieldRenderer` does these:
...
`FieldRenderer` simply bridges a single field's **event stream** into **React** — _nothing more._
::
...
- It **simply connects** a single `FormField` instance to **React** — _and gets out of the way._

### 🧠 1. Resolves & Registers the Field

:::doc-ref-section
It calls `useFormField()` internally. This:

```tsx
useFormField(form, path, { defaultValue, overrideInitialValue });
```

- Resolves the **field** using the **strongly-typed path**
- **Registers** it inside the `FormController`
- Applies `defaultValue` if **needed**
- **Optionally** overrides **initial data** (controlled by `overrideInitialValue`)
...
It **does not**:
- **Subscribe** the **entire form**
- **Trigger** parent **re-renders**
- **Depend** on **React context reactivity**

It **reacts strictly** to **mutations** that affect:
- **This field**
- Or **its descendants** _(for nested structures)_
:::
...
By default, fields are **not destroyed**.
