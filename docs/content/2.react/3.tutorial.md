...
seo:
  title: Goodie Forms React Tutorial — Build a Typed Signup Form with Validation
  description: Step-by-step tutorial on using Goodie Forms with React to create a type-safe signup form. Learn useForm, field hooks, validation with Zod, and fine-grained reactivity without unnecessary re-renders.
---
...
- How validation works with `zod` schemas (or [any other Standard Schema schemas](https://standardschema.dev/schema#what-schema-libraries-implement-the-spec)).
...
::doc-ref-section
We can enforce strong validation using `zod` (or [any other Standard Schema-compatible library](https://standardschema.dev/schema#what-schema-libraries-implement-the-spec)).

See the [Validation](/core/validation) page for more information on how validation works and how to integrate with other libraries.
...
This is _intentional_ — it keeps your components performant and avoids unnecessary re-renders.
...
::doc-ref-section
At this point, you've seen how each primitive solves a specific problem.
Now let's look at how they work together to build a real, production-ready form flow:
...
- `useForm()` creates a **centralized**, **reactive** form state and wires up **validation** and **submission** behavior.
  ...
  - `form.watchEvent()` lets a component subscribe to a **specific internal form lifecycle event** — enabling _fine-grained reactivity_ and _custom behavior_ built on top of the controller's _event system_.
- `useFormField()` gives you full programmatic control over a **specific field** — including _setting_, _modifying_, _focusing_, and _inspecting its state_.
...
Together, these **primitives** let you build **complex**, **high-performance** forms with _explicit reactivity_ and _minimal boilerplate_ — while keeping behavior **predictable** and **scalable**.
::
